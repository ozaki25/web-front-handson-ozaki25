...
- [ ] 章のキャップストーン（「TODO アプリ」「アプリを仕上げる」「デプロイ」等）の **後ろに新規レッスンを足していない** か。足すなら synthesis の **前** に挿入する
- [ ] 「N 章 の総合演習 / 集大成」のように章の集約を主張する文言は、後から章の構造を変えると壊れるので **書かない / 書いたなら章末に固定する**
- [ ] 章末を主張する文言（「1 章 全体のまとめ」「次の N 章 へ進む」等）は **章の最後尾レッスン 1 箇所のみ**。中間レッスンに書くと後続を追加した瞬間に壊れる
