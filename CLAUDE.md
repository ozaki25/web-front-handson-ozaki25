...
2. **追加先の章を決め、その章の末尾の番号 + 1 を新レッスン番号にする**（例: 1 章 が `lesson01〜lesson14` なら `lesson15`）。
3. 後続の章の番号がずれる場合は、**この時点で `/renumber-lessons` を回して全体を章内連番に戻す**。「とりあえず章末に飛び番号で追加」は禁止。
4. 対象技術の最新版を確認し、本文中のバージョン表記を合わせる。
