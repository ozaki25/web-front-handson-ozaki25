...
> **次のパターンは commit 時に hook が自動検出するので意識不要**: 絵文字・装飾記号 / 位置依存参照（「次のレッスン」「前のレッスン」「先ほど」）/「章 N」表記 / 省略コード / 用語揺れ（「プロップス」「疑似クラス」）/ 素の `<script>` `<style>` / 太字の閉じが全角閉じ括弧の直後。詳細は `.claude/hooks/validate-branch-commit.sh` を参照。
