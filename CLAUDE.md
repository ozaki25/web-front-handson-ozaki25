...
2. **追加先の章を決め、その章の末尾の番号 + 1 を新レッスン番号にする**（例: 章 1 が `lesson01〜lesson14` なら `lesson15`）。
...
- 1 レッスン内でのレベル跳ね上がり。
- `…(省略)…` を使ったコード片。
- **位置依存の参照**: 「次のレッスンでは」「前のレッスンで」「先ほどの〜」のように **読者の現在地に依存する表現** は使わない。代わりに **「○○のレッスン」「章 X の〜」** のように topic-name で参照する。レッスン番号や順序が変わっても壊れないため。
...
- [ ] 追加先の章で **連番** になっているか（例: 章 1 が 1-14 のところに lesson106 を追加してはいけない）
