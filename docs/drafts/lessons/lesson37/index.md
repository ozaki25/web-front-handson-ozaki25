...
lesson36 でカウンター（数値）の state を扱いました。配列の state でも考え方は同じですが、**守らないといけないルール** が 1 つあります。
...
`filter`（lesson20）は**新しい配列**を返すので、そのまま渡してよいです。`prev` 自体は変更されません。
...
配列ではなく、**オブジェクト** を state に入れたときの更新（`setUser((prev) => ({ ...prev, age: 30 }))` など）も同じ発想でできます。ただし覚える量を分散させるため、本コースでは **lesson42（React 版 TODO のミニ統合）で扱います**。このレッスンでは配列更新だけに集中します。
...
1. StackBlitz の React + Vite（TS）テンプレートから新規プロジェクトを作る（lesson36 のを使い回しても OK）
2. `src/types.ts` を作成
