...
章 2 までで、HTML に `<script defer>` で JS を読み込み、`document.querySelector` で要素を取ってきて `textContent` や `classList` をいじって画面を書き換えてきました。章 2 の仕上げの lesson25 では、TODO アプリを「入力欄 + 一覧 + 削除 + `localStorage` 保存」まで作りました。
...
この `Hello` をアプリのどこかで `<Hello />` と書くと、その場所に `<h1>Hello, React</h1>` が展開されます。自作の HTML タグを増やしていくようなイメージです。JSX の詳しい書き方は lesson33 で扱います。
...
2. 「React + Vite + TypeScript」テンプレートを選んで新しいプロジェクトを作る（テンプレート名は微妙に違うことがあります。`react-ts` や `React TypeScript` などを選べば OK）
3. 左のファイルツリーから `src/App.tsx` を開く
4. 中身を下記の内容に書き換える
5. 右のプレビューで確認する
...
`{name}` の部分は JSX の中に JS の変数を埋め込む書き方です（詳しくは lesson33）。
...
- `const name = "Alice";` の `"Alice"` をあなたの名前に書き換えて保存する。保存するとプレビューが自動で更新されることを確認する
