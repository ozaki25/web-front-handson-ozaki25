...
`useState(initialValue)` の `initialValue` に **関数を渡す** と、その関数は **コンポーネントの初回レンダーのときだけ** 実行される。ここで `localStorage` から読み込む。

- 初回レンダーで `todos` にはすでに復元済みの配列が入っている
...
export default function App() {
  // ❌ このバグあり版では useState([]) にして useEffect で後から読み込む
  const [todos, setTodos] = useState<Todo[]>([]);
...
- `useState([])` の `[]` の代わりに **関数** を渡している（`() => { ... }`）。この関数は初回レンダーのときだけ実行される
