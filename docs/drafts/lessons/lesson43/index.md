...
- Server Component: サーバーで動く。データベース接続やファイル読み込みなど、秘密情報を扱える。`useState` や `onClick` は使えない。
- Client Component: ブラウザで動く。`useState` / イベント / ブラウザ API が使える。先頭に `"use client"` と書いて明示する（詳しくは lesson46）。
