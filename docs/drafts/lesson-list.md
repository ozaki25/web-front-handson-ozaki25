...
v4 → v5（+22、75 レッスン）→ v6（スコープ洗練）→ v7（サイクル 2 指摘反映で確定版）。

## v6 → v7 の主な変更（サイクル 2 レビュー 3 視点を反映）

- **lesson66 / lesson73**: Next.js 15 の `params` / `searchParams` は **`Promise<...>` 型**で `await` 必須。備考に明記
- **lesson69**: React 19 で `useFormState` が **`useActionState` にリネーム**され、import 元が `react-dom` → `react` に変わった旨を備考に追記。`useFormStatus` は `react-dom` のまま
- **lesson57 バグあり版の具体化**: 3 種類のバグを固定 — (a) 空文字 TODO を弾き忘れる、(b) toggle で別の TODO も巻き添え（イミュータブル更新ミス）、(c) `localStorage` 初期化タイミングで空配列を上書き
- **lesson52 `ref` props 化**: 「React 19 以降は `ref` を普通の props として渡せる」で言い切り。`forwardRef` の過去形は扱わない境界を明示
- **lesson56 Profiler**: StackBlitz の「Open in New Tab」で Profiler が動くか実機確認する運用を明記。不可なら **CodeSandbox か StackBlitz の full-window プレビュー** を代替動線に
- **既存本文への追記チェックリスト**: lesson04 / lesson21 / lesson25 / lesson38 / lesson47 / lesson55 / lesson59 / lesson68 の 8 箇所を独立タスクとして番号振り替え節に列挙
- **双方向螺旋**: lesson21 末尾に lesson55 予告、lesson04 末尾に lesson64 予告、lesson25 末尾に lesson43 予告、lesson38 末尾に lesson47 予告
- **lesson65 `next/font`**: 期待出力に「適用前後のスクショ対比」を必須化、デフォルト `display: "swap"` に 1 行触れる
- **lesson72 環境変数**: 失敗例演習を必修化（`NEXT_PUBLIC_` なしの変数をクライアントで読むと `undefined` になることを見る）、本番対比の予告 1 段落
- **lesson61 Route Groups**: `(app)/layout.tsx` のサイドバーが `/todos` だけに出て `/about` には出ない **対比スクショ** を期待出力に明記
- **lesson74 Tailwind**: 演習は「`create-next-app --tailwind` の観察」に絞る（コード書き換えは発展）
- **lesson70**: `isTodo` シグネチャを lesson37 と固定、サーバー側 POST 検証とクライアント側 fetch 検証の役割分離
- **lesson70 Edge 方向修正**: 「Middleware は既定が Edge、Route Handlers は `export const runtime = 'edge'` を書かない限り Node.js」と明示
- **lesson64**: `width` / `height` は「原則必須。静的 import / `fill` の場合は省略可」に精緻化
- **lesson33 末尾**: 「次の lesson34 で `interface` を紹介するが、本コースは `type` を基本に使う」を宣言
- **lesson47**: Strict Mode 二重呼び出しで副作用バグが顕在化する点を 1 行注記（純粋関数原則の補強）
...
- **新概念**: `useRef<HTMLInputElement>(null)` / `ref={inputRef}` / `inputRef.current?.focus()` / 値保持用途（`useRef<number>(0)` で再レンダリング時も値が残る）/ `useState` との違い（`useRef` を変えても再レンダリングは起きない）/ **React 19 以降は `ref` を通常の props として渡せる**（`function MyInput({ ref }: { ref: Ref<HTMLInputElement> })` の形）。
- **過去形の扱い**: `React.forwardRef` を使った古い書き方は **本コースでは扱わない** と明示する（React 18 以前のサンプルに当たった読者が混乱しないように境界を引く）。
...
- **期待出力（対比）**:
  - `/about` にアクセス → ルートレイアウトのナビだけ表示、サイドバーは **出ない**
  - `/todos` にアクセス → ルートレイアウトのナビ + `(app)` のサイドバーが **両方出る**
  - スクショでこの差分を読者に見せる（ただし本文では ASCII 図で代替可）
...
- **新概念**: `import Image from "next/image"` / `<Image src width height alt>` / `width` / `height` は **原則必須。ただし静的 import（`import heroImg from "./hero.png"`）の場合と `fill` プロップを使う場合は省略可** / ローカル画像のインポート / `next.config.ts` の `images.remotePatterns`。
...
- **新概念**: `next/font/google`（`import { Inter } from "next/font/google"`）/ `next/font/local`（`import localFont from "next/font/local"`）/ `font.className` を `<body>` や `<html>` に付与 / 自動で最適化（self-host、サブセット）される仕組み / **`display` のデフォルトは `"swap"`**（FOUT で表示、システムフォントが一瞬見える）。
- **演習**: `Inter` を読み込んで `app/layout.tsx` で `<html className={inter.className}>` に適用。日本語対応の `Noto Sans JP` も使い分ける。
- **期待出力（必須）**: 適用前（システムフォントのゴシック）と適用後（Inter）の **見た目のスクショ対比** を本文に掲載。読者が「変わった」と目視確認できる形にする。
- **スコープ外**: `variable` font / `display` の他の値 / CSS 変数連携は扱わない。
...
- **演習**:
  1. `app/api/todos/route.ts` で `GET`（一覧 JSON を返す）と `POST`（JSON ボディから 1 件追加）を実装
     - **サーバー側検証**: `POST` 内で `await request.json()` した値を **`isTodoLike(input)`** で検証する（章 3 lesson37 の `isTodo` と同じシグネチャを再利用）
  2. ブラウザの DevTools Console から `fetch('/api/todos')` して動作確認
  3. **クライアント側検証**: 受信した JSON を **`isTodoArray(data)`** で検証し、失敗時はエラー表示する小さな Client Component を書く（章 3 lesson37 の `isTodo` を import して使う）
  4. 役割分離を冒頭で 1 段落書く: 「サーバー側は入力検証、クライアント側は受信検証。両方で型ガードが役立つ」
- **螺旋**: 章 2 lesson27 `fetch` が章 5 で「自分のサーバーを叩く」形で再登場。章 3 lesson37 型ガードが **サーバー側と Client 側の両方** で使われる。
- **ランタイム注記（本文で明確化）**:
  - **Middleware は既定が Edge ランタイム**（Node API は使えない）
  - **Route Handlers は既定が Node.js ランタイム**（`export const runtime = 'edge'` を書かない限り）
  - 両者のランタイムが違うことを初学者が混同しないよう、1 段落で対比する
...
- **演習**:
  1. `NEXT_PUBLIC_APP_NAME` と（あえてプレフィックスなしの）`APP_SECRET` の 2 つを `.env.local` に書く
  2. Server Component で両方 `process.env.XXX` として表示 → どちらも読めることを確認
  3. **失敗例**: Client Component（`"use client"`）で `process.env.APP_SECRET` を読むと `undefined` になることを確認。これが `NEXT_PUBLIC_` プレフィックスの意味（クライアントに露出する / しない）を体感する瞬間
  4. 本番対比の予告: Vercel の Environment Variables に設定する手順は lesson75 でまとめて扱う旨を末尾に書く
...
- **演習**: `create-next-app` で `--tailwind` 付きの新規プロジェクトを試しに作り、デフォルトで設定されている Tailwind v4 の `@import "tailwindcss";` と `postcss.config.mjs` の `@tailwindcss/postcss` プラグインを **観察するだけ** にする。
  - 本コースで作ってきた自己紹介 / TODO プロジェクトに Tailwind を **持ち込まない**（本コースの素 CSS 資産を壊さないため別プロジェクトで試す）
  - 余力があれば、`bg-slate-100 p-4 text-lg` のようなデフォルトパレットのクラスで小さな要素を 1 つ書いてみる（任意、`@theme` によるカスタムは扱わない）
...
### 既存レッスンの刷新スペック（統合 2 本）

#### lesson57: TODO アプリを React で作る（ミニ統合、刷新版）

- **必修スコープ**:
  1. lesson55 で抽出した `useTodos()` カスタムフックを起点にする
  2. `useTodos` に **localStorage 連携** を内蔵（初期値関数で読み込み、`useEffect` で保存）
  3. 「バグあり版 → 修正版」構造にする
- **仕込む 3 つのバグ（執筆時に必ずこの 3 つに絞る）**:
  - **バグ A**: 空文字 TODO を弾き忘れる（`addTodo("")` が通ってしまう）→ `trim().length === 0` で弾くよう修正
  - **バグ B**: `toggleTodo(id)` で全 TODO の `done` が反転してしまう（イミュータブル更新ミス、同じオブジェクト参照で `done = !done` を書いた結果）→ `prev.map(t => t.id === id ? {...t, done: !t.done} : t)` で修正
  - **バグ C**: `useState([])` で初期化してから `useEffect` で localStorage を読み込むため、初回レンダリング直後の保存用 `useEffect` が空配列で上書きしてしまう（lesson42 で経験済みのパターン、ここで再確認）→ `useState(() => JSON.parse(localStorage.getItem("todos") ?? "[]"))` の初期値関数で修正
- **発展（折りたたみ）**:
  - A. `useTodos` を Context でアプリ全体に提供する（lesson51 Context の応用）
  - B. `useReducer` で書き直す（lesson47 の応用、Action 型は `{ type: "add" | "delete" | "toggle", ... }`）
- **想定時間**: 60〜90 分（必修のみ）、発展を両方やる場合は 120 分超になるため「別日の挑戦」と明記

#### lesson73: 小さなアプリを仕上げる（Next.js 統合、拡張版）

- **主役スコープ**:
  1. `searchParams` を **`await` して受け取る**（`Promise<{ q?: string }>` 型）
  2. `generateMetadata` で記事詳細ページの動的タイトルを設定
- **既習機能の復習（新規タスクではなく、完成形でそのまま使う）**:
  - ルートレイアウト（lesson60）とナビ
  - `/about`（lesson59 で作成済み）
  - `/todos`（lesson68-69 Server Actions で作成済み、TodoForm はそのまま）
  - 画像は `next/image`（lesson64 で差し替え済み）、フォントは `next/font`（lesson65 で設定済み）
- **演習の着地点**: 「TODO 一覧ページに検索フォーム `?q=...` を置き、`searchParams.q` で絞り込む」「詳細ページのタブタイトルが記事タイトルになる」の 2 点が体感できれば完了
- **必ず扱う注記**: Next.js 15 で `params` / `searchParams` は `Promise<...>` になり、`await` しないと警告（Next.js 16 で破壊的変更予定）
- **スコープ外**: Route Groups の書き換えはしない（lesson61 で完成済みのものを使う）

## 既存本文への追記タスク（番号振り替え時の必須チェックリスト）

| # | 対象ファイル（v7 番号）| 追記内容 |
|---|---|---|
| 1 | lesson04 末尾 | 「本物のアプリでは `<img>` を `<Image>` コンポーネントに置き換える。章 5 lesson64 で再訪」 |
| 2 | lesson21 まとめ | 「`makeFilter` の形は章 4 lesson55 のカスタムフックで再登場する」 |
| 3 | lesson25 末尾 | 「ファイル分割の発想は章 4 lesson43 のコンポーネント分割で再登場する」 |
| 4 | lesson33 末尾 | 「次の lesson34 で `interface` を紹介するが、本コースは `type` を基本に使う」を宣言 |
| 5 | lesson38 まとめ | 「この判別共用体パターンは章 4 lesson47 の `useReducer` の `Action` 型で再登場する」 |
| 6 | lesson47 冒頭 | 「この `Action` 型は章 3 lesson38 で学んだ判別共用体そのもの」（再接続） |
| 7 | lesson55 冒頭 | 「章 2 lesson21 の `makeCounter` と同じ仕組みを React の文脈で使う」（逆参照） |
| 8 | lesson59 末尾 | 「ここで使った `<img>` は章 5 lesson64 で `<Image>` に差し替える」 |
| 9 | lesson68 戻り値 | Server Action の戻り値型を `{ ok: true } | { ok: false, error: string }` で判別共用体として明示 |
