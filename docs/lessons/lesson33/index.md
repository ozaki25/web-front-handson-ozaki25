...
章 2 の「オブジェクト」で、オブジェクトを使って「人」や「TODO」のような複数の値をまとめました。TS ではそのオブジェクトの **形** を型で書けます。
...
章 2 の「TODO アプリを作る」で、TODO 1 件を次のようなオブジェクトで表しました。
...
- 章 2 の TODO の `{ id, text }` を `Todo` 型として `types.ts` にまとめた。この型は「配列・ユニオン・リテラル型・オプショナル」で `status` と `memo?` を足して育てていく。
- 次のレッスンでは、配列の型 `T[]`、ユニオン型 `|`、リテラル型、オプショナルプロパティ `?` を扱う。

次のレッスンで `interface` を紹介しますが、本コースは `type` を基本に使います。両方を学んだうえで `type` で統一する、という流れで進めます。
