...
章 2 lesson22 で、オブジェクトを使って「人」や「TODO」のような複数の値をまとめました。TS ではそのオブジェクトの **形** を型で書けます。
...
章 2 lesson30 で、TODO 1 件を次のようなオブジェクトで表しました。
...
このレッスンでは、`id` と `text` の 2 プロパティだけの最小の `Todo` 型を作ります。「未完了 / 完了」の状態や「メモ」のような追加情報は次のレッスン（lesson35）で足していきます。

### このレッスンで扱わないこと

オプショナルプロパティ（`name?: string` のような `?` 付きプロパティ）は lesson35 で扱います。このレッスンでは **全プロパティが必須** の形だけを書きます。
...
次の lesson34 で `interface` を紹介しますが、本コースは `type` を基本に使います。両方を学んだうえで `type` で統一する、という流れで進めます。
