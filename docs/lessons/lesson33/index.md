...
lesson32 で、コンポーネントは JSX を返す関数だと紹介しました。
...
「`for`（`<label for="...">`）→ `htmlFor`」も同じ仲間ですが、実用頻度が上がるのは章 5（フォーム）なので、ここでは頭の片隅に。
...
`if` で分岐したいときは三項演算子や `&&` を使います（lesson39 で扱います）。
...
1. lesson32 の StackBlitz をそのまま使う（別プロジェクトを新規作成しても OK）
