...
この後のレッスンで「Vite の仕組み」「ESLint / Prettier / Biome」「次世代ツールチェイン」など個別ツールを深掘りしますが、その前に「**何をするツールがあるか**」「**なぜこれだけ種類があるか**」の見取り図を整理します。
...
ややこしいのは、1 つのツールが **複数の役割を兼ねる** ことです。Vite はバンドラですが、内部で Rolldown / Oxc を使ってトランスパイルもします（Vite 8 より前は esbuild）。Biome はリンターとフォーマッタを兼ねます。Oxc は将来的にバンドラ以外のすべてを統合する目標を持っています。
...
ES2015 の登場で言語が大きく変わり、ブラウザごとの対応がバラバラになったため、トランスパイルが必須になりました。
...
ブラウザの ESM サポートが普及し、「開発時はバンドルしない」選択肢が現実的になりました。さらに JS 製ツールの遅さを Go / Rust で解決する流れが始まりました。
...
- **Oxc**（2023〜）: 高速 JS / TS パーサ + リンター + フォーマッタ + リゾルバの統合プロジェクト。**oxlint** v1.0（2025 年 6 月）は ESLint より圧倒的に速く、Shopify / Airbnb などの主要 OSS が採用
...
「**Vite + TypeScript + ESLint + Prettier + Vitest**」が現時点の最大公約数で、リンターを oxlint や Biome に置き換える選択肢も実用域に入っています。
