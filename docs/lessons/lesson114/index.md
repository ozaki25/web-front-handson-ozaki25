...
- **元に戻せる**: 「あ、消したけどやっぱり要る」「3 日前の状態に戻したい」がすぐできる
- **誰がいつ何を変えたか分かる**: バグの原因を「いつ入った？」で追跡できる
- **複数人で並行開発**: 各自が **ブランチ** で作業し、最後に合体できる
- **PR ベースの開発**: コードレビューを経てマージする現代的なフローの土台

2026 年現在、ほぼすべての開発現場で Git が使われており、Git を扱えないと業務に参加すること自体が難しいレベルの基本になっています。
...
`.git/` ディレクトリが作られ、Git で管理できるようになります。既存のプロジェクトを Git 管理下に置きたい時も同じです。
...
`-m` でコミットメッセージを指定します。**他のメンバー + 未来の自分** が読めるよう、何のための変更か簡潔に書きます。
...
`switch` はブランチ切り替え専用の新しいコマンドです。`checkout` でも同じことができますが、`checkout` は他の用途（ファイル復元など）も兼ねるので役割が分かれた `switch` の方が明確です。
...
これで `feature/login` の変更が `main` に取り込まれます。**競合がなければこれだけで済みます**。競合がある場合の対処は次の節で説明します。
...
`origin` は **リモートの名前** です。慣習でリモートは `origin` と呼ばれます。
...
`-u` は upstream 設定で、初回のみ必要です。次回以降は `git push` だけで OK です。
...
リモートの履歴をローカルに取り込みますが、まだ自分のブランチには適用しません。中身を確認してから merge / rebase したい時に使います。
