...
3. **実ユーザー**: 可能ならスクリーンリーダーなどの支援技術を日常的に使う人に触ってもらう → リアルなフィードバック
...
スクリーンリーダーは、macOS なら OS 付属の **VoiceOver**、Windows なら無料の **NVDA** が使えます。自分で触ってみると、文章の説明だけでは分からなかった体験が一気につかめます。
...
Playwright の導入はテスト入門のレッスンで扱うので、そこと組み合わせて CI に足せます。こうしておくと、`aria-label` や `alt` の欠落に PR の段階で気付けるようになります。`expect(results.violations).toEqual([])` をいきなり全ルールで通すと、新しいルールが追加されたときに **無関係の PR が落ちる** ので、`withTags` でスコープを絞り、`disableRules` で既知の課題を一旦よけてから順次解消するのが定石です。
...
- `axe DevTools` の **Intelligent Guided Tests**（有料版機能）の存在を知っておく。自動チェックで拾えない項目を、質問形式で人間の確認を誘導しながらチェックしてくれる
...
- a11y は「セマンティック HTML」「ARIA とキーボード操作」「自動チェック」の 3 つをセットで押さえる
