...
### 1 章の「フォームを作る」の HTML フォームとの対比

1 章の「フォームを作る」の自己紹介フォームを思い出してください。次のような形でした。
...
どちらも「フォームを作る」ことには違いありません。使い分けの目安は、HTML ネイティブで十分なら前者、入力値を**その場で JS から使いたい**（リアルタイムプレビュー、バリデーション、条件付き無効化など）なら後者です。React では制御コンポーネントが基本です。
...
送信ボタンを押したときの処理には、2 通りの書き方があります。

#### (a) `onSubmit` で書く（本コースの 4 章はこれ）
...
#### (b) `<form action={fn}>`（紹介のみ、5 章で本格使用）
...
4 章の段階では **紹介のみ**です。5 章の「Server Actions の最小形」でこの形を実際に使うときに「関数を渡すパターン」を詳しく学びます。「4 章は `onSubmit` 形、5 章で `action={fn}` 形に乗り換える」という流れを予告として覚えておいてください。
...
- 4 章内では `<form onSubmit={(e) => { e.preventDefault(); ... }}>` の形を「親子コンポーネントの連携」で使い続ける
- `<form action={fn}>` 形は 5 章の「Server Actions の最小形」で登場する。そこで `preventDefault` が不要になる理由とあわせて扱う
...
- フォーム送信は当面 `onSubmit` + `e.preventDefault()`。`<form action={fn}>` は 5 章でフル活用
