...
2. 特定の値が変わるたびに実行（依存配列 `[todos]` など）: **次の lesson57** で localStorage 保存に使う
...
`todos` が変わるたびにタブのタイトルを更新する例です。配列の**中身**が変わっただけでなく、**配列オブジェクト自体**が前回と違う必要があります。lesson46 で学んだ「スプレッドで新しい配列を作る」イミュータブル更新が、このシグナルとして効きます。
...
- `count` の代わりに、lesson50 で作った `todos` state を使って、`document.title` を `TODO (${todos.length})` に更新する
- `<input>` で文字を入れて追加ボタンで増やすと、タブのタイトルの件数が増える、という挙動を作る
- lesson57 の準備として、ファイルを保存しておくと役立つ
...
- 次の **lesson57** では、`useEffect` を **localStorage への保存** に使います（依存配列に `todos` を渡す形）。lesson30 で作った「リロードしても消えない TODO」を、React 版でも取り戻します
