...
「タグ → カンマで props → カンマで子要素を並べる」と読むと、JSX の見た目と関数呼び出しの構造の対応がつかめます。下のデモは CDN から React を読み込み、**変換後の `React.createElement` を直接書いて** 同じ結果を出しています。タグみたいに見える JSX の正体がこの関数呼び出しだ、というイメージを掴んでください。
