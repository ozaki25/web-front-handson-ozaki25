...
| `sticky` | 親の中で、スクロール位置に応じて「通常配置」と「固定」が切り替わる |
...
ここで **親に `position: relative` を書き忘れると**、`.badge` は更に上の祖先（もしくは `<body>`）を基準にしてしまい、カードの右上ではなくページ全体の右上に飛んでしまいます。これはよく起きる事故なので、「絶対配置の子がいる親には `position: relative`」と覚えます。

また、`absolute` の要素は通常の流れから抜けるため、元の場所に隙間は残らず、兄弟要素が詰めて配置されます。ここが（元の場所を空けたままにする）`relative` との違いです。
...
「`z-index: 9999` を付けたのに何故か前に出ない」場面に将来出会います。原因は **stacking context** という仕組みです。要点だけ言うと、**ある要素に `position` + `z-index` を指定すると、その要素の内部に「独立した重なり順のグループ」が作られ、外側の `z-index` と直接比較できなくなる** というものです。
...
`.card { position: relative; }` の 1 行を **一時的に削除** してみてください。`.badge` はカードを基準にできなくなり、**ページ全体の右上**（ビューポートではなく `<body>` の端）に飛びます。この挙動から、`absolute` が親要素を基準に位置決めしていることが分かります。確認できたら、`position: relative` を戻します。
...
3. `.to-top` と `.badge` の `z-index` を入れ替えてみる。2 つが画面上で重ならない限り見た目は何も変わらないことを確認する。`z-index` は要素同士が実際に重なったときにだけ意味を持つ。
