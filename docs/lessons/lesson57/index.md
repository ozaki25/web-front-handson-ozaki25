...
スプレッド構文 `...`（2 章の「分割代入とスプレッド」）は、このイミュータブル更新で頻出します。
...
順番を入れ替えるだけで、最新のものを上に表示したいときに使います。
...
`filter`（2 章の「配列の変換」）は**新しい配列**を返すので、そのまま渡してよいです。`prev` 自体は変更されません。
...
パターンは「`XXXEvent<HTML要素>`」で一貫しています。型が分からなくなったら、IDE の補完を見るか、いったんインラインで書いて推論された型を確認するのが手早い調べ方です。
