...
- lesson59 で `/about` に貼った `<img>` を `<Image>` に置き換えて、自動最適化の恩恵を受けられます。
...
- **縦横比で起きる「ガタつき」**: 画像の読み込みが終わるとレイアウトがズレて、読んでいた本文がピョンと下に動きます（CLS）。
...
lesson63 までで作った Next.js プロジェクトがあればそのまま使えます。手元に無ければ、新規 StackBlitz の Next.js テンプレート（<https://stackblitz.com/fork/github/vercel/next.js/tree/canary/examples/hello-world>）を開き、下の「出発点のファイル」を貼って揃えてください。`/about` の画像を差し替える演習なので、最低限 `/about` ページと `<img>` が存在すれば構いません。
...
lesson59 〜 lesson63 で作ってきた StackBlitz プロジェクトを開き直しましょう。lesson61 の Route Groups 化を済ませていれば、`/about` のファイルは `app/(public)/about/page.tsx` にあります。
...
`app/(public)/about/page.tsx`（lesson61 以前のままなら `app/about/page.tsx`）を開きます。lesson59 で貼った 3 枚のカードの `<img>` を、`<Image>` に置き換えます。
...
lesson59 の `about.css` には次のような指定が入っていました。
...
ダークモードでの `filter` 調整などは不要です。lesson59 時点の CSS のままで構いません。

### 期待出力

1. ブラウザで `/about` を開きます。見た目は lesson59 時点とほぼ同じです（カード 3 枚にプレースホルダ画像）。
...
- lesson59 の `<img>` を `<Image>` に差し替えたことで、WebP / AVIF 変換や遅延読み込みの恩恵を自動で受けられるようになりました。
- 次の lesson65 では、フォントの読み込みを `next/font` で最適化します。画像に続き「ブラウザがガタつかずに表示できる」状態を目指します。
