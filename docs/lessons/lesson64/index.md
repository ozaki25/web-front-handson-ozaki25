...
::: tip このレッスンで扱う範囲
本コースで扱う `useEffect` は 2 パターンだけです。

1. マウント時に 1 回だけ実行（依存配列 `[]`）
2. 特定の値が変わるたびに実行（依存配列 `[todos]` など）。代表用途は **state の変化を localStorage に保存する**

クリーンアップ関数 / `addEventListener` / `setInterval` などは本コースのスコープ外です。必要になった時点でドキュメントを調べてください。
:::
