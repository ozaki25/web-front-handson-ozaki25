...
Node.js は 22 で `.ts` ファイルの直接実行（型注釈などを実行時に剥がす）を実験的に導入し、23.6 でフラグなしの既定になりました。これに合わせて `verbatimModuleSyntax`（TypeScript 5.0）や `erasableSyntaxOnly`（TypeScript 5.8）といった「ネイティブ実行向きの書き方を強制する」オプションが整い、TypeScript 6 / 7 のフレームワーク標準として広がっています。このレッスンもこれらを前提にした設定で書いています。
:::
...
| `strictBuiltinIteratorReturn` | 組み込みイテレータの戻り値を厳密に型付け（TS 5.6 で strict 入り） |
| `useUnknownInCatchVariables` | `catch (e)` の `e` を `unknown` 型に |

なお `alwaysStrict`（出力に `"use strict"` を付ける）は、TypeScript 6 / 7 では常時有効になり、`strict` の切り替え対象からは外れました。

**新規プロジェクトでは必ず `"strict": true`** を付けます。後から ON にする方がずっと大変です。
...
特に `noUncheckedIndexedAccess` は `arr[0]` の型を `T | undefined` にします。バグを防げる一方、書き味が硬くなります。新規プロジェクトでは最初から入れ、既存プロジェクトに追加するなら計画的に進めるのがよいでしょう。
...
新規 React / Next.js プロジェクトでは `"jsx": "react-jsx"` です（Next.js は tsconfig 生成時にも起動時にも `react-jsx` を入れます）。

### `lib`

「**どの実行環境の API の型を有効にするか**」。
...
「**型チェックだけして JS を出力しない**」設定。バンドラが TS → JS の変換を担当する場合は `noEmit: true` にします。Vite / Next.js は `tsc` を使わず esbuild / SWC で変換するので、ほぼ常に `noEmit: true` にしておきます。

`tsc` を **型チェック専用ツール** として使う、というのが現代の TS の主な役割です。

### TypeScript 6 / 7 時代の前提オプション

Node.js が `.ts` をネイティブ実行する時代に合わせて、**「型情報以外を含まないコード」を強制する** オプション群（`verbatimModuleSyntax` は TypeScript 5.0、`erasableSyntaxOnly` は 5.8）が標準になりました。`npm create vite@latest` / `create-next-app` の生成 tsconfig にも入っているので、現代のフロントでは「常に ON」と思って構いません。
...
#### Vite + React（`npm create vite -- --template react-ts` の生成例。細部は Vite のバージョンで変わります）
...
- `allowArbitraryExtensions` で `.css` などの拡張子付き import をバンドラ前提で許可
...
- `jsx: "react-jsx"`（Next.js が `react-jsx` を強制する。JSX 変換は Next.js のビルドが担う）
...
→ 依存パッケージの型が壊れていると `node_modules` の中まで型チェックして失敗します。実用上は `true` にしておくのがほぼ常識になっています。

#### Next.js の `jsx` を別の値に変えたい

→ Next.js は tsconfig 生成時に `"jsx": "react-jsx"` を入れ、起動時にも `react-jsx` を強制します。手動で変えても上書きされるので、基本はそのままにします。
...
`@tsconfig/strictest` や `@tsconfig/node20` のような **公式バンドル** も npm にあり、`extends: "@tsconfig/strictest/tsconfig"` と書くだけで強い設定一式が読み込めます。
...
このまま `npx tsc --noEmit` を実行すると、`strict: true` が効いているためエラーが出ます。
...
- `lib` で使える型、`types` で読み込む型パッケージを制御する。
