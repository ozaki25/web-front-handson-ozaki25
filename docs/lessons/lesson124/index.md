...
フィーチャーフラグの大きな利点のひとつがこの Kill Switch です。本番で問題が起きた時に、**コードを巻き戻さず** に機能を即 OFF にできます。

- 「決済 v2 を ON にしたら障害」→ 管理画面で OFF に
- 「新検索が API を叩きすぎ」→ OFF に

ロールバック（git revert + 再デプロイ）に **数分** かかるのに対し、フラグ OFF なら **数秒** で止められるため、本番運用の安心感が大きく変わります。

### フラグの寿命管理

フラグが増えるとコードが if 分岐だらけになり、見通しが悪くなります。**寿命を管理** する習慣が大事。
