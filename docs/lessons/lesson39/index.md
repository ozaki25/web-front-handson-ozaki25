...
const observer = new ResizeObserver((entries) => {
  for (const entry of entries) {
    ...
    target.classList.toggle('tall', height > 180);
  }
});
...
下のデモは `contenteditable` の枠に文字を入れたり改行したりすると、`width` / `height` が即時に更新される最小例です。改行を重ねて高さが 180px を超えると枠の色が変わります（`classList.toggle("tall", height > 180)` の効果）。
...
#target.tall { background: #c7f3d0; border-color: #16a34a; }
@media (prefers-color-scheme: dark) {
  #size-info { color: #ccc; }
  #target { background: #3a2f0a; color: #f5f5f5; border-color: #b8860b; }
  #target.tall { background: #0a3a1f; border-color: #4ade80; }
}
  `"
  :js="resizeDemoJs"
/>
