# lesson106: Core Web Vitals の 3 つの指標と Lighthouse
...
「3 つすべて Good」が合格ラインです。1 つでも Poor があると、ユーザー体験に問題があると判断されます。
...
- 5 章で扱った `next/image` は `width` / `height` を必須にすることで CLS を構造的に防ぐ
...
3. レポートが表示される
   ...
   - INP は「Interactions」レーンに各クリックの遅延が出る
...
- RUM は `web-vitals` ライブラリで自前実装できる。Vercel ホスティングなら Speed Insights で手軽に導入できる
