...
章 2 の lesson20 では、数値を合計する関数を書きました。
...
関数型を使うと「関数を受け渡すときの契約」を型で書けるようになります。章 2 lesson24 の `map` / `filter` に渡す関数も、実は TS のライブラリ側で関数型が定義されています。
...
省略できる引数を作る方法は lesson35 でオプショナルプロパティと合わせて扱います。
