...
主役は `compilerOptions`。それ以外に「どのファイルを対象にするか」を `include` / `exclude` で指定します。
...
- `tsconfig.json` の主役は `compilerOptions`。`include` / `exclude` で対象ファイルを絞る
