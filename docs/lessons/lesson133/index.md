# lesson133: PWA の manifest とインストール
...
「Service Worker のライフサイクルとキャッシュ戦略」で学んだ Service Worker を使って、ネイティブアプリに近い機能を追加します。
...
多くの PWA では `standalone` を選びます。`browser` にするとインストールしても通常のブラウザタブと同じ見た目のままなので、インストールを促す意味が薄れます。
...
<img src="/diagrams/web-push-flow.svg" alt="Web Push の 3 者構成: ①ブラウザが Push Service に購読してエンドポイントを取得しサーバーに保存。②あなたのサーバーが VAPID 署名付きで Push Service に送信。③Push Service がブラウザへ配信し SW が受信して通知を表示する。" class="diagram" />
...
「Service Worker のライフサイクルとキャッシュ戦略」の演習で作った `pwa-sample` プロジェクトを使います。そこでは `vite-plugin-pwa` の `manifest` オプションで自動生成していましたが、ここでは静的ファイルを手動で配置する形を確認します。

アイコンは同じ演習の手順で `public/pwa-192.png` と `public/pwa-512.png` を置いている前提です。まだ無ければ、任意の 192×192 / 512×512 PNG を同名で配置してください（無くても manifest 自体は読めますが、DevTools のアイコンプレビューが壊れた画像になります）。
