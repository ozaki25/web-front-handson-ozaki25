# lesson125: Content-Security-Policy（CSP）実践
...
そのため `'unsafe-inline'` 抜きの厳格な CSP を当てるには **nonce 方式** が必要になります。
:::
...
未対応ブラウザ（Safari）でも壊れずに、対応ブラウザでさらに守りが厚くなります。2026 年は **新規プロジェクトに Trusted Types を入れる** ことが推奨されています。
...
CSP と一緒に **これら 5〜6 個も設定** するのが現代の標準になっています。Vercel ダッシュボード / Cloudflare の管理画面で **テンプレート** が用意されています。
