...
章 2 lesson24 では、素の JS で `<form>` の送信を止めるために `event.preventDefault()` を書きました。React（章 4）でも `onSubmit` の中で同じことをしていました。
...
- 戻り値はありません（あっても無視されます。戻り値を使いたいときは lesson51 の `useActionState`）。
- 関数は **必ず `async`** です。
...
- **StackBlitz や Vercel でサーバーが再起動すると消えます**。本物の永続化には DB が必要ですが本コースでは扱いません（lesson53 末尾でも再度注意を書きます）。
...
- 送信中のボタン無効化、空入力エラー表示は **lesson51** で追加します。本レッスンでは最小形に集中します。
- `Todo` ごとの削除ボタンも本レッスンでは扱いません（lesson52 の統合で扱います）。
...
- 次の lesson51 では、送信中の状態表示とエラー表示を `useActionState` / `useFormStatus` で追加します。`addTodo` のシグネチャもそこで少し変えます。
