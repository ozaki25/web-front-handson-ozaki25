...
- 「特定の値のみ許す」 **リテラル型** を書ける。
- 省略可能なプロパティ `?:` を書ける。
- lesson28 の `Todo` 型に `status: "open" | "done"` と `memo?: string` を足して、育てた `Todo` 型で配列を扱える。
...
- `Array<number>`: 「`Array` という型に `number` を流し込んだもの」。ジェネリクス（lesson30 で扱う）の書き方。
...
- `Todo` 型を `status` と `memo?` まで育てた。この形のまま lesson30 / lesson31 でも使う。次章でも `import type` して再利用する。
