# lesson38: 監視系 API（Intersection Observer / ResizeObserver / MutationObserver）
...
メディアクエリは画面全体しか見ません。`@container`（lesson15）は CSS だけで分岐できますが、**JS 側で処理を変えたい** 場合は ResizeObserver の出番です。
