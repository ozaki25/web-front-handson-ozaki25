...
- `<link>` は `<img>` と同じく **中身を持たない（void 要素）ため終了タグを書かない** タグ。`<link rel="stylesheet" href="...">` でも `<link rel="stylesheet" href="..." />` でも動く（`/` は HTML では無視され、好みで書ける）。
...
- 別のレッスンで、同じ要素の中でも「このボタン」「このリンク」のように一部だけ装飾を変えたいときに使う **クラスセレクタ** を扱う。
