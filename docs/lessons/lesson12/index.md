...
これで `.card` 同士の間に 16px の空間が入ります。`margin` でも間隔は作れますが、行の両端にまで余白が付いてしまいます。`gap` は要素と要素のあいだにだけ隙間を入れるので、Flexbox では `gap` が基本です。
...
このレッスンではステップ 1 で HTML / CSS を全面的に書き直します。これまでのプロジェクトの続きでも、新規の StackBlitz Vanilla テンプレート（<https://stackblitz.com/edit/web-platform>）でも構いません。手元にこれまでのプロジェクトが無い場合も、下の「出発点のコード」を貼る必要はなく、新規プロジェクトのままステップ 1 に進んで構いません。
...
ここまでの **期待出力**: 自己紹介ページが、カードも含めて「縦に積まれた」状態で表示されます。ヘッダーとフッターはダークブルー背景、本文は薄いグレー背景です。ダークモードの見え方は、DevTools の Rendering パネルで `prefers-color-scheme: dark` をエミュレートして確認できます（開き方は「Transition と hover アニメーション」で詳しく扱います）。
...
本レッスンではまず Flexbox の使い方を体験するために、カードも Flex で横並びにしています。ただし「カードを画面幅に応じて折り返しながら敷き詰める」のは **2 次元レイアウト** なので、実務では CSS Grid を使うのが自然です。「CSS Grid で 2 次元レイアウト」で、この `.cards` 部分を Grid に書き換え、`@media` を書かなくても自動で折り返す形に育てます。
:::
