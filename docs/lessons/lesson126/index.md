# lesson126: Server Components の設計論
...
これを誤って Client Component から import するとビルド時にエラー。**シークレットの漏洩防止** に有効（lesson115）。
...
- React Compiler（lesson125）と組み合わせて、`useMemo` を消した状態で動かす
