# lesson126: 依存性セキュリティ（npm audit / Dependabot）
...
「**標準で書ける** ことを依存追加で済ませていないか」を再検討します。`Date.now()` / `fetch` / `Intl` / `Set` など、**ブラウザ / Node 標準で書ける** ものは依存を入れない方がよい。
...
- 「**標準で書けるなら依存しない**」という方針が、サプライチェーンリスクを減らす最も確実な手段
