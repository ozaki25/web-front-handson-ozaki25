...
- `rel="noopener"` は **新しく開いたタブから元のタブを操作されないようにする** 安全策。これを書かないと、リンク先のページが JavaScript で「`window.opener` 経由で元のタブを別の URL に飛ばす」ような乗っ取りができてしまいます。**現代のブラウザは `target="_blank"` のリンクで自動的にこの保護を有効化** しますが、明示しておくと古い環境やブラウザ外（メーラー / Electron 等）に渡る HTML でも安心なので、書く癖をつけておきます。
