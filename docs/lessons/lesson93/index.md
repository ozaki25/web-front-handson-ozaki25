...
- **IndexedDB**: より大容量のデータベース系 API。本コースでは扱わないので、こういうものがあると知っておけば十分
...
「どこで重くなっているか」「レンダリングが遅い原因は何か」を特定するための基本の道具です。近年の Performance パネルは、記録した内容から改善のヒント（Insights）を自動で提示してくれるので、まずはそれを眺めるところから入るとハードルが下がります。

### Lighthouse タブで Core Web Vitals を測る

Performance タブの録画は、詳細まで追える代わりに **見方を覚える必要** があります。最初に手を出すべきは Performance タブの隣にある **Lighthouse タブ** です。`Analyze page load` を押すだけで、ページに対して点数とレポートが出ます。
...
Lighthouse は手元の環境（あなたの PC）で測るので、本番の数値とは少しずれます。本番の実ユーザー指標は **Google Search Console** の Core Web Vitals レポートや **`web-vitals` ライブラリ**（内部で `PerformanceObserver` を使う）で取得します。
...
- **`Ctrl+Shift+F` / `Cmd+Opt+F`**: 全ソース横断検索（Sources タブ）。ライブラリのコードも含めて全文検索できる
...
`Ctrl+Shift+M` / `Cmd+Shift+M` でツールバー上部にデバイス選択が出ます。`iPhone SE (375×667)` / `Pixel 7` / 任意サイズ（Responsive）で表示を切り替えられます。1 章の「Flexbox とレスポンシブ」や「CSS Grid」の演習で使ったはずです。
...
- Application タブで進捗データのキーを削除してリロードすると、該当レッスンの完了マークが消える
- Sources タブで貼ったブレークポイントの行で、リロード時に JS が一時停止する
