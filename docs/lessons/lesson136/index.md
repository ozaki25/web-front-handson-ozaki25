# lesson136: Web Analytics（Vercel Analytics / GA4）
...
**ヨーロッパ** からアクセスがある場合、Cookie を使う Analytics は **同意バナー** が必要です。日本の個人情報保護法も「Cookie 類による行動データの第三者提供」に同意取得を要求するケースが増えています。
...
Safari の ITP は **3rd-party cookie を実質ブロック**、1st-party cookie も **7 日で失効** させます。Chrome は **2024 年 7 月に 3rd-party cookie の一律廃止計画を撤回** し、さらに **2025 年 4 月には予定していた「ユーザーに選択させるプロンプト」も取りやめて**、既存の Cookie 設定を維持する方針に落ち着きました（強制ブロックは進めない）。とはいえ Safari の挙動と合わせて長期的には **「3rd-party cookie に依存しない設計」** が必要なのは変わりません。
