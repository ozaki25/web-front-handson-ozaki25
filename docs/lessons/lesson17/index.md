...
モーダルや折りたたみは、かつては JavaScript やライブラリで作り込むものでした。いまは HTML 単独で同じものを作れる要素が揃っています。JS を書かなくても、Tab キーや Escape キーが効き、スクリーンリーダーも正しく読み上げてくれます。
...
アクションメニュー / ツールチップ / 通知トースト / 設定パネルのような「**モーダルじゃない** けれど出し入れしたい UI」のための API です。2024 年に全主要ブラウザ対応（この状態を **Baseline** と呼びます）となり、2026 年現在は安心して使えます。
...
外側をクリックすると自動で閉じる挙動を **ライトディスミス** と呼びます。

| 値 | 閉じる条件 |
|---|---|
| `popover` or `popover=\"auto\"` | **外側をクリック**、Escape、他の auto popover が開いた時に閉じる |
| `popover=\"manual\"` | **自分で close を呼ばない限り閉じない**。toast 向き |
| `popover=\"hint\"` | tooltip 用。ライトディスミスあり、auto と manual の中間の挙動。ただし Safari 未対応で新しめ（実験的） |
...
- 一覧から項目を消す前に出す「本当に削除しますか？」の確認を `<dialog>` で作る
...
::: tip この章のドリルで力試し
[1 章「HTML / CSS」のドリル →](/quiz/chapter1/) で、4 択問題で理解度を確認できます。回答履歴はブラウザに保存されるので、途中で閉じても続きから再開できます。
:::
