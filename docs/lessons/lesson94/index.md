...
- 強キャッシュ（期限内はサーバーに聞かない）と弱キャッシュ（サーバーに聞くが中身は省略）の違いを説明できる
- `ETag` / `Last-Modified` と `304 Not Modified` の関係を説明できる
- キャッシュ起因の「新しいデプロイが反映されない」という定番のトラブルを避ける実務パターンを知る

## 解説

### キャッシュが必要な理由

ブラウザがリソース（HTML / CSS / JS / 画像）を毎回サーバーまで取りに行くと、時間もかかり通信量もかさみます。一度取ったものを **同じなら再利用する** のがキャッシュです。効き方の強い順に、次の 3 段階があると覚えると整理しやすいです。
...
| `no-cache` | 使う前に毎回サーバーに問い合わせる（弱キャッシュは効く）。ボディは返ってこないこともある |
...
- **`no-cache`**: キャッシュ **は** する。ただし使う前に毎回サーバーに聞く（変わっていなければ `304` が返りボディ転送はゼロ。後述の「弱キャッシュ」で詳しく扱う）
- **`no-store`**: キャッシュ **しない**
...
DevTools の Network タブで見ると、`Size` 列が **`(memory cache)`** や **`(disk cache)`** になり、サーバーに行かなかったことが分かります。
...
`max-age` が切れるまでは、コードをいじってデプロイし直しても **古いファイルが配られ続けます**。キャッシュ起因の「デプロイしたのに反映されない」問題は、ここから発生します。

### 弱キャッシュ: `304 Not Modified`
...
- `Hard reload`（`Ctrl+Shift+R` / `Cmd+Shift+R`）を試す。`Disable cache` と同様だが、DevTools が閉じていてもキャッシュを無視できる
