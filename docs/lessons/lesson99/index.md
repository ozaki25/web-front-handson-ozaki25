...
ARIA（Accessible Rich Internet Applications）は、HTML だけでは表現しきれない意味をスクリーンリーダーに伝えるための **追加の属性** です。W3C は **ARIA の 5 つの原則** として、使いどころを絞るための次のルールを示しています（抜粋）。
...
つまり ARIA は最後の手段として位置付けられています。まずは `<button>` / `<nav>` / `<h1>` / `<label>` のような意味あるタグを使い、それでも足りないとき（複雑なウィジェット・動的更新・ネイティブ要素がないもの）だけ ARIA を足します。
...
**`tabindex` の正の数値は原則使わない**ようにします。DOM の登場順に従うのが一番自然で、壊れにくいからです。
...
- モーダルは `<dialog>.showModal()` を使うと、ブラウザがフォーカストラップを自動でやってくれる
