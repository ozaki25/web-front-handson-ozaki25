# lesson130: WebSocket と Server-Sent Events（SSE）
...
「サーバー側から **自発的に**（プッシュで）データを送りたい」場面の主な選択肢:
...
「**双方向**、**バイナリも送れる**、**HTTP からアップグレード** して始まる」プロトコル（`ws://` と `wss://`）。
...
- **通知 / 株価 / AI ストリーム / ライブニュース**: SSE で十分
