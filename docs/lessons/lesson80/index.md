...
- `error.tsx` が担当する範囲と、Server Actions のフォームエラー（「useActionState でフォームエラーを返す」）の担当範囲が **別物** であることを理解する
...
::: tip `notFound()` は HTTP ステータスも 404 にする
単に「見つかりません」と画面に書くだけだと、HTTP ステータスは **200 OK** のままです。検索エンジンは 200 を「正常に表示されているページ」と判断し、**「見つかりません」のページをインデックスに入れてしまう** リスクがあります（= ソフト 404）。`notFound()` を呼ぶと Next.js が **正しく 404 ステータス** を返すため、検索エンジンに「このページは存在しない」と伝わります。SEO の観点でも `notFound()` は重要です。
:::
...
Server Actions のフォームエラーは、例外を投げる代わりに **戻り値でエラー情報を返す** 設計になっています。「useActionState でフォームエラーを返す」で扱う `useActionState` がその受け皿です。フォーム送信のエラーを `error.tsx` に落とそうとしても動かないので、混同しないでください。
...
| フォームの入力エラー | 「useActionState でフォームエラーを返す」の `useActionState`（戻り値） |
...
- 「データが見つからない」ときは `notFound()` を呼び、`not-found.tsx` を表示する（HTTP ステータスも 404 になる）
- 「レンダリング中の例外」は `error.tsx` で受け止める（`"use client"` 必須）
- フォーム入力エラーはこの 2 つでは扱わない。「useActionState でフォームエラーを返す」の `useActionState` を使う
