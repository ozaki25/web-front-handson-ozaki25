...
- 2 章 の TODO アプリの 1 件を表す `Todo` 型を作り、関数で受け取れる。
...
このレッスンでは、`id` と `text` の 2 プロパティだけの最小の `Todo` 型を作ります。「未完了 / 完了」の状態や「メモ」のような追加情報は「配列・ユニオン・リテラル型・オプショナル」で足していきます。

### このレッスンで扱わないこと

オプショナルプロパティ（`name?: string` のような `?` 付きプロパティ）は「配列・ユニオン・リテラル型・オプショナル」で扱います。このレッスンでは **全プロパティが必須** の形だけを書きます。
...
`Todo[]` は「`Todo` の配列」を意味する型注釈。別のレッスンで詳しく扱うが、ここでは「配列の各要素が `Todo` 型」とだけ理解しておけばよい。
...
- 2 章 の TODO の `{ id, text }` を `Todo` 型として `types.ts` にまとめた。この型は「配列・ユニオン・リテラル型・オプショナル」で `status` と `memo?` を足して育てていく。
- 別のレッスンで、配列の型 `T[]`、ユニオン型 `|`、リテラル型、オプショナルプロパティ `?` を扱う。

別のレッスンで `interface` を紹介しますが、本コースは `type` を基本に使います。両方を学んだうえで `type` で統一する、という流れで進めます。
