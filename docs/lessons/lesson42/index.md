...
章 4 の仕上げです。lesson25 で素の JS + DOM で作った TODO アプリを、章 4 で学んだ React + TS に全面移植します。さらに **localStorage で保存・復元する螺旋** を閉じます。章 4 で扱わなかった **オブジェクト state のイミュータブル更新** もここで扱います。

想定時間は **60〜120 分** です。焦らず段階的に組み立てましょう。

## ゴール

- lesson25 の TODO アプリを React + TS に移植し、3 つのコンポーネント（`TodoInput` / `TodoList` / `TodoItem`）に分割できます。
...
章 2 の lesson25 で、素の HTML + JS + localStorage で TODO アプリを作りました。構成は次のようなものでした。
...
3. **配列の更新はイミュータブル**: `push` / `splice` は使わず、新しい配列を作ります（lesson37 で学びました）。

### 使う型（章 3 の `Todo`）

章 3 lesson28 〜 31 で `types.ts` に次の型を育ててきました。
...
**状態の持ち主** は一番上の `App` です。`TodoInput` は「追加しました」を `onAdd` で伝えるだけ。`TodoList` と `TodoItem` は描画と削除イベントの伝達だけを担います（lesson40 で学んだ state lifting の応用です）。
...
`try` / `catch` で囲んでいるのは、`localStorage` に不正な JSON が保存されていた場合（何かの事故で壊れた場合）に `JSON.parse` が例外を投げるためです。lesson22 で学んだ `try` / `catch` の復習になっています。

### オブジェクト state のイミュータブル更新

lesson37 では配列の state 更新（`[...prev, newItem]` など）を扱いました。オブジェクトを state にするときも同じ発想が必要になります。
...
StackBlitz のトップから「React + Vite + TypeScript」を選びます（もしくは lesson41 までで作ったプロジェクトをそのまま使っても構いません。本レッスンは新規プロジェクトの方が整理しやすいです）。`npm install` と `npm run dev` は自動で実行されます。
...
lesson29 で追加した `status` と `memo` は今回は使わないので省略します。章 5 で Server Actions 版に移植するときに拡張します。
...
lesson38 で扱った制御コンポーネントの形です。`onSubmit` で `preventDefault()` を呼んでいます（章 5 lesson50 の Server Actions ではこれが不要になります）。空文字列は追加しません。
...
`import type` で `Todo` 型を取り込んでいます（lesson28 の形）。`onDelete(todo.id)` で親に削除要求を渡します。
...
lesson35 の `.map` + `key` パターンです。`key={todo.id}` を忘れないでください。`todos.length === 0` のときの早期 return は lesson39 で学んだ条件表示の形です。
...
- 章 2 lesson25 の素の JS + DOM の TODO が、React + TS + コンポーネント分割 + localStorage + `useEffect` で書き直せました
...
- lesson44 で章 1 の自己紹介ページを `/about` として復活させます
- lesson47 でデータ取得を Server Component に任せます（ブラウザ側 `fetch` + `useEffect` の罠を避けられます）
- lesson50 で **Server Actions** を使い、今回書いた `onSubmit` + `preventDefault()` の代わりに `<form action={serverAction}>` で送信をサーバーに届けます
- lesson52 で TODO アプリが一覧 + 詳細 + 追加フォームを備えた「実用っぽい」形になります

本レッスンの成果物（`App.tsx` / `TodoInput.tsx` / `TodoList.tsx` / `TodoItem.tsx` / `types.ts`）は、章 5 lesson50 で Server Actions 版に書き換えるときの **出発点** になります。StackBlitz のプロジェクトは残しておくか、ローカルにコピーしておきましょう。
