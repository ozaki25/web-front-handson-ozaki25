...
「CSS を当てる」で `color` と `line-height` は先に紹介しましたが、ここでまとめて扱います。
...
最近はどの OS でも読みやすい既定のフォントが入っているので、「各 OS の既定フォントを使う」という指定もよく見ます。便利なので、本コースの演習でもこの指定を使います。
...
`color` や `font-size`、`font-family` は **子要素に継承** されます。つまり `body` にフォントを設定すれば、その中のすべてのテキストにも同じフォントが（上書きしない限り）適用されます。便利な性質ですが、「書いた覚えのないスタイルが当たっている？」と感じたら、親から継承されたスタイルを DevTools の Styles パネルで確認してみます。
