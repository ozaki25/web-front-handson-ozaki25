...
2 章で書いた配列に型を付けていきます。配列の型は 2 通りの書き方があります。どちらも意味は同じです。
...
- `number[]`: 「数値の配列」。短くて読みやすいので普段はこちらを使う。
...
「この変数には `"open"` か `"done"` か `"archived"` のどれかしか入らない」ということが型で書けます。2 章で文字列リテラルを比較していた部分（`if (status === "done")` など）が、typo まで含めて TS が守ってくれるようになります。
...
呼び出して結果を Console に出す。使える道具は 2 章で学んだ `for...of`、`filter`、`length` など。どれを使っても構わない。

### 本コースで扱わないもの（`enum` / `as const`）
