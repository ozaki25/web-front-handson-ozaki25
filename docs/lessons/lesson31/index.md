...
`await` のいちばんの効能は、**非同期な処理を「上から順に書く」同期処理のように扱える** ことです。本来の非同期はコールバックや `.then` の連鎖で「結果は後で来るから、こうなったらこうしてくれ」と書く必要がありましたが、`await` を使えば結果が返ってくるまでその行で止まったように書けます。
...
上のコードは見た目には「`start` を出す → 1 秒待つ → `end` を出す」という **同期処理と同じ読み方** で理解できます。実際は `await` の行でブラウザ全体は止まっていないのですが、関数の中だけを見れば「順番に実行される」と読み下せる、というのが `await` を使う利点になります。
...
### 演習用ヘルパー: `wait(ms)` を定義する

`await` の動きを確認するために、「○ミリ秒待つ」Promise を返す関数を自分で定義します。標準の JS にはないので、演習ファイルに毎回書き加えて使います。中身は今は気にしなくてよいです（`setTimeout` を Promise で包んでいる、とだけ覚える）。
