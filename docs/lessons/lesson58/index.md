...
- Client Component: ブラウザで動きます。`useState` / イベント / ブラウザ API が使えます。先頭に `"use client"` と書いて明示します（詳しくは lesson62）。
...
- `app/layout.tsx`: 全ページ共通の外側の枠（`<html>` と `<body>` の中身）。lesson60 で触ります。
- `app/page.tsx`: 今書き換えたトップページ。
- `package.json`: 依存パッケージと `scripts`。`"dev"`, `"build"`, `"start"` などが並んでいます。

`app/` 以下にディレクトリを作って `page.tsx` を置けば、それがそのまま URL になります。これを次の lesson59 で実際にやります。
...
- 次の lesson59 ではページを増やして `<Link>` で行き来し、章 1 の自己紹介ページを `/about` として復活させます。
...
ブラウザの「ソースを表示」で見える HTML は、初回表示用に別で生成された HTML です。ページ遷移（lesson59 の `<Link>`）では、RSC ペイロードだけが追加で送られてきて、必要な部分だけがツリーに差し替わります。
