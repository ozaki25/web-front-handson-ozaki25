...
`Date` は **ミリ秒数** を内部に持っているだけのシンプルな型です（1970 年 1 月 1 日 00:00:00 UTC からの経過ミリ秒、いわゆる **Unix タイム × 1000**）。表示形式（日本式 / ISO 形式 / 西暦・元号など）は別の話で、後述の `toLocaleString` や `Intl.DateTimeFormat`（lesson41）で整形します。
...
タイムスタンプは数値なので **引き算ができる**（後述の差分計算）、**比較も簡単**（`a < b` で先後判定）、**localStorage への保存も楽**（数値を文字列化するだけ）です。「日時を保存しておきたい」場面では `Date.now()` を取っておくのが定番。
...
`setMonth` は「2 月 31 日」のような無効な日を **自動で繰り上げ** ます。これが想定外の動きを生むことがあるので、月またぎの計算は注意が必要。
...
細かい整形（曜日も入れる、時刻だけ、相対表現「3 分前」など）は `Intl.DateTimeFormat` を使います。lesson41 で扱います。
...
**入力文字列は ISO 形式（`YYYY-MM-DD` または `YYYY-MM-DDTHH:mm:ss`）に統一する** のが鉄則。

### `day.js` の出番

純正 `Date` だけで頑張れる範囲は次のようなもの。
