...
この `Action` 型は章 3 の「判別共用体」で学んだ **判別共用体** そのものです。`type` プロパティで種類を見分ける形、`switch` での分岐、網羅性チェック、すべてそのまま使います。章 3 の「判別共用体」で書いた `TodoState` の代わりに、今回は「どういう更新をしたいか」を表すオブジェクトを同じ仕組みで表現します。
...
action は「操作の種類」を表すオブジェクトです。`type` というプロパティで種類を見分けます。章 3 の「判別共用体」で学んだ判別共用体が、そのまま action の型になります。
...
章 3 の「unknown と never」で登場した `never` 型を使うと、**action の case を書き忘れたときにコンパイルエラーにできます**。
...
- action は判別共用体で型付け（`type` プロパティで見分ける、章 3 の「判別共用体」の形）
