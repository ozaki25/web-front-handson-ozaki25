...
「`unknown` と `never`」では、`unknown` で受けた値は **そのままでは何もできない** ことを確かめました。このレッスンでは、その値を絞り込んで **使える形** に変える道具を学びます。
...
「`unknown` と `never`」で後回しにした「`unknown` を絞り込む具体的な方法」が、この型ガードです。
...
- 型ガードは「**実行時の確認を通して、TS に型を絞り込ませる**」ための仕組み。
- `typeof` はプリミティブ（`string` / `number` / `boolean` など）を判定する。`typeof null === "object"` の落とし穴に注意。
- `Array.isArray` は配列かどうかを専用に判定する。
- `in` はオブジェクトに特定のプロパティがあるかで判定する。
