# lesson129: PWA の通知・インストール・manifest
...
lesson128 で作った `pwa-sample` プロジェクトを使います。lesson128 では `vite-plugin-pwa` の `manifest` オプションで自動生成していましたが、ここでは **静的ファイルを手動で配置する形** を確認します。両方ある場合、`<link rel="manifest" href="...">` で指定したパスのファイルが優先されます（プラグインの自動生成は `<link>` を後から書き換えない限り上書きされません）。

アイコンは lesson128 の手順で `public/pwa-192.png` / `public/pwa-512.png` を置いている前提です。まだ無ければ、任意の 192×192 / 512×512 PNG を同名で配置してください（無くても manifest 自体は読めますが、DevTools のアイコンプレビューが broken image になります）。
