...
自分でメトリクス調整を書く必要はありません。`<link>` 直書きや他のフォント読み込み手段では CLS 対策を手動でやる必要があるのに対し、`next/font` ではこの対策が自動で付くため、本コースでは採用しています。
...
+---------------------------------------+
|  自己紹介                             |  ← 日本語はシステムフォント
|  Web フロントエンドを学び中です。    |     「Web」の部分は Inter で表示
|                                       |
|  HTML / CSS / JavaScript              |  ← ラテン文字は丸みのある Inter
+---------------------------------------+
```
...
### 手順 3: 日本語は `Noto_Sans_JP` をヘッダーに当てる

日本語の本文も Web フォントにしたい場合は、`Noto_Sans_JP` を併用します。ここでは「ヘッダーだけ `Noto_Sans_JP`、本文は Inter + システム日本語フォント」の使い分けをやってみます。
...
ヘッダーの文字が Noto Sans JP の字形になり、本文と少し印象が変わります。ただしヘッダーに日本語は入っていないため、差分は欧文（Home / About / Todos）の字形で見ることになります。
