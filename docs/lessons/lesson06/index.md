...
- `<label for>` でラベルと入力欄を紐付けられる（クリックでフォーカス移動）。
- `name` 属性で「送信時にどのキーで値が出ていくか」を指定できる。
- `required` / `type="email"` などブラウザネイティブのバリデーションを使える。
...
`<form>` には送信先（`action`）やメソッド（`method`）といった属性もありますが、後のレッスンで順に使っていくので、ここでは `<form>` の中に入力欄を並べる形だけ扱います。
...
ボタン。フォームの中に置いた `<button>` は、既定でフォームを送信する動きになります。
...
- `submit`: フォームを送信する（既定値）
- `reset`: フォームの入力内容をクリアする
- `button`: 既定の送信動作を持たない。JS から使うときに指定する

送信ボタンなら `type="submit"` を明示的に書くのが分かりやすいです。
...
### もう 1 つの書き方: `<label>` で入力欄を囲む（参考）

`<label>` には、`for` を使わずに入力欄を **中に入れて囲む** 書き方もあります。他のサイトのコードで見かけることがあるので、形だけ知っておきます。

```html
<!-- 上の節で扱った書き方（このコースではこちらを使う） -->
<label for="username">お名前</label>
