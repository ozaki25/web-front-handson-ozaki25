# lesson132: PWA の manifest とインストール
...
lesson131 で作った `pwa-sample` プロジェクトを使います。lesson131 では `vite-plugin-pwa` の `manifest` オプションで自動生成していましたが、ここでは静的ファイルを手動で配置する形を確認します。

アイコンは lesson131 の手順で `public/pwa-192.png` / `public/pwa-512.png` を置いている前提です。まだ無ければ、任意の 192×192 / 512×512 PNG を同名で配置してください（無くても manifest 自体は読めますが、DevTools のアイコンプレビューが壊れた画像になります）。
