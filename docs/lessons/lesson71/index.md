...
lesson70 の Route Handlers は既定で Node.js、Middleware は既定で Edge。この違いは覚えておいてください。
...
1. lesson70 のプロジェクトを開く（またはこれまでの章 5 プロジェクト）
...
lesson70 で作った `/api/todos` も、middleware で認証必須にできます。
