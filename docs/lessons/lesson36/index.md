...
- 具体的な型ガードで `unknown` を絞り込む書き方は **次のレッスン** で扱うと理解する。
...
この「絞り込む」具体的な方法は **次のレッスンで扱う型ガード** に委ねます。本レッスンでは、`unknown` が「絞り込まないと何もできない型」であることだけを体験します。
...
// raw.name と書きたくても書けない。次のレッスンで絞り込み方を学ぶ。
```
...
`unknown` を **どう絞り込むか**（`typeof` / `in` / カスタム型ガード）は、次のレッスンで扱います。ここでは「絞り込まないと何もできない」こと、そして「`never` で網羅性を検査できる」ことだけ押さえればじゅうぶんです。
...
`raw` を `unknown` 型で受けると、中身にアクセスする前に「絞り込み」が必要になる。この絞り込みを次のレッスンで `typeof` や `in` や自作の型ガード関数で書けるようになる。
...
- `unknown` の **具体的な絞り込み方**（`typeof` / `in` / カスタム型ガード）は **次のレッスン** で扱う。
