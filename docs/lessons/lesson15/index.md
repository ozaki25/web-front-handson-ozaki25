# lesson15: モダン CSS（:has と Container Queries が主役）

## ゴール

このレッスンは 2022 〜 2025 年に追加されたモダン CSS の入り口です。**主役は次の 2 つ** です。

- `:has()` で **親セレクタ**（子の状態に応じて親を装飾）を書ける
- `@container`（Container Queries）で **親要素の幅** に応じたレイアウトを書ける

主役を体感したら、次の機能も「読めるようになる」を目標にしてください（本文の **補足** で扱います）。
...
**初回は主役 2 つで「JS でやっていたことが CSS だけでできる範囲が広がっている」を体感** すれば十分。残りは「実際に必要になったときに本レッスンの補足を見直す」運用で OK。
