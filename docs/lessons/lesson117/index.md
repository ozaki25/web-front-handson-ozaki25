...
「ボタン 1 つでデプロイ」が **Continuous Delivery**、「main にマージしたらそのまま本番へ」が **Continuous Deployment** にあたり、どちらも略称は CD です。
...
Next.js なら `.next/cache` を保存すると **増分ビルド** が効いて高速になります。

::: warning キャッシュの落とし穴
キャッシュ key の設計がずれると **古いキャッシュを掴んで不可解な失敗が起きる** ことがあります。`package-lock.json` のハッシュを必ず key に含め、想定外の挙動が出たら手動で **キャッシュを削除** します。
:::

### マトリクスビルド

複数の Node.js バージョン / OS で同時にテストする時に便利です。
...
これだけで **OS 3 種 x Node 2 種 = 6 並列** のテストが走ります。OSS ライブラリなどで重宝します。
...
`if: github.ref == 'refs/heads/main'` で **main 限定** にします。
...
- `pull_request_target` を使うと fork PR でも secrets が読めますが、**fork の悪意あるコードがそのまま走るため極めて危険** です。利用は「ラベル付けや welcome コメント等、コードを実行しない処理に限る」ように徹底します
...
を設定できます。**本番デプロイに人手の承認を入れる** のに便利です。
...
組織内で **同じ workflow を複数リポジトリで使う** 場合は、**reusable workflow** が便利です。
...
ESLint 設定 / 簡単な test は省略しても構いません。
...
- **CI** は統合のたびに自動でテストする仕組み、**CD** は自動でリリースする仕組み。**パイプライン** はその段階を並べたもの
- **lint / typecheck / test を並列**、build は `needs:` で待たせるのが基本形
- **キャッシュ**（`actions/setup-node@v6` の `cache: npm` や `actions/cache`）で大幅に高速化
