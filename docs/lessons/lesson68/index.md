...
- **ローカルで `npm run dev`** を走らせる（StackBlitz と DevTools が噛み合わないときの最終手段）

本コースはブラウザ完結を基本方針にしていますが、DevTools の挙動は環境差が出やすい領域です。うまく動かなければ本レッスンの演習はスキップしてもらって構いません（機能理解だけ押さえて先に進めます）。
...
- ツールバーの歯車 → General で「Highlight updates when components render.」を有効にすると、**再レンダリングした要素の周囲が一瞬光る** ようになります。最初に試したい観察ツールです
...
3. 画面で以下を 1 回ずつ押す:
   ...
   - 「色を切り替え」
4. 停止ボタン（■）を押す
5. Commit タブに 3 件の Commit が記録されているはずです
