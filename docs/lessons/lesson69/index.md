...
ここで登場するのが **ErrorBoundary** で、境界より内側で起きた例外を受け止めてフォールバック UI（エラー表示）に差し替える仕組みです。境界の外側は無事なままです。
...
`<Suspense>` は ErrorBoundary と対になる仕組みで、**非同期なデータ / コンポーネントを待つ間、フォールバック UI を見せる** 役割を担います。
...
- `fallback` の中身を自由な JSX に差し替えて、見た目を変える（例: `<div><h3>読み込みエラー</h3><p>あとで試してください</p></div>`）
