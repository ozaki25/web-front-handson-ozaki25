...
lesson68 の `addTodo` は、空入力のとき「何もしない」で終わりでした。これではユーザーに「空だから弾いた」ことが伝わりません。
...
lesson68 の `addTodo` は `(formData) => void` の形だったので、ここで書き直します。
...
lesson68 で自分で作った `/memo` ページを、同じ要領で「空入力エラー + 送信中無効化」に対応させましょう。`addMemo` の引数を `(prevState, formData)` に書き直し、`<MemoForm />` を Client Component として切り出し、`SubmitButton` で `useFormStatus` を使います。
...
- 次の lesson73 では、ここまでの知識を統合して TODO アプリを仕上げます。詳細ページ・メタデータ・`searchParams` によるハイライト表示を足します。
