...
- **サーバー側だけで動く**: `useState` / `useEffect` / `onClick` などブラウザ専用の機能は使えない
- **サーバー側でしかできない処理が書ける**: データベース接続、ファイル読み込み、API キーの利用など
- **コンポーネントの JS そのものはブラウザに送られない**: レンダリング結果だけが届くので、ブラウザが受け取る JS が小さくなる

逆に、ボタンクリックや state 管理など **ブラウザでしかできない操作** が必要な部品は、別途 **Client Component**（クライアントコンポーネント）として明示的に切り出します。先頭に `"use client"` と書くだけで Client 扱いになり、4 章 までの React と同じ書き味で動きます。

実際のページは **Server Component と Client Component を混ぜて構築する** のが普通です（「ヘッダーや一覧はサーバー側、検索ボックスやドロップダウンはクライアント側」のような使い分け）。詳しい組み合わせ方は「Server Component と Client Component」のレッスンで扱います。
