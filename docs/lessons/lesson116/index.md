...
- **`steps`**: 順番に実行する処理
  - `uses: actions/checkout@v7` のような形: 既製のアクションを使う
  - `run: ...`: シェルコマンドを実行
...
緑のチェックは通過、赤は失敗を示します。OSS では事実上の必須アイテムとして扱われます。
...
| アクション | 用途 |
|---|---|
| `actions/checkout@v7` | リポジトリを checkout |
| `actions/setup-node@v6` | Node.js セットアップ |
| `actions/cache@v6` | 任意のディレクトリをキャッシュ |
...
ローカルで `npm run test:run` を実行し、`PASS src/sum.test.ts` のような出力が出ることを確認します。
...
GitHub のリポジトリで PR を作成します。

PR ページに **「Some checks haven't completed yet」** が出て、しばらくすると **「All checks have passed」** に変わるはずです。**Details** リンクから個別の job ログを見られます。Vitest の出力（`PASS sum > 1 + 2 = 3` のような行）が Actions ログに表示されます。
...
- `actions/cache@v6` で `~/.cache/Cypress` などをキャッシュして E2E を速くする
