...
代償は速度と安定性です。E2E は本物のブラウザを起動するぶん遅く、ネットワーク事情で fail することもあります。そのため、対象を最重要パスだけに絞って運用するのが基本になります。
...
> **待ち合わせを安定させる小技**: ローディング表示に `role="status"` または `aria-busy="true"` を付けておき、`getByRole('status')` で待ち合わせると、見た目の文言やスタイルが変わってもテストが壊れにくくなります。
...
最初に E2E を書くときは UI モードを使うのを強くおすすめします。「どこでクリックすればいいか」「次の状態は何か」を見ながら書けるので、習得が一気に楽になります。
...
が入っており、`npx playwright show-trace trace.zip` で UI モードと同じインターフェースで再生できます。CI で起きた fail を後からローカルで再現できる点が大きな強みになります。
...
「すべての画面を網羅する」ような E2E は頻繁に壊れ、メンテナンスコストが膨らみます。**ビジネスが止まる経路だけ** を 20〜30 ケースくらい用意して守るのが現実解です。
...
- ローディング表示に `role="status"` を付けて `getByRole('status')` で待ち合わせると、見た目が変わってもテストが安定する
