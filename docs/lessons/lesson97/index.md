...
> **`user=alice` は仕組み説明用の架空値**: 上の例では分かりやすさのために `user=alice` という平文を入れていますが、**実際のサービスでは Cookie にユーザー名のような値を直接入れません**。代わりに、サーバーが発行した **それ自体には意味のないセッション ID**（例: `session=abc123xyz...`）を入れて、サーバー側でその ID から「これは alice さんのセッション」と引き当てる形が一般的です。本文の以降の例（`session=abc123` など）はこの実運用の形を想定しています。
...
| JS からの読み書き | `document.cookie`（`HttpOnly` なら JS からは見えない） | `localStorage.setItem` 等 |
| 有効期限 | 属性で制御 | localStorage は恒久、sessionStorage はタブを閉じると消える |
...
主要ブラウザ（Chrome / Firefox）の **デフォルトは `Lax`** です（Safari は挙動が異なります）。クロスサイトで明示的に送りたい場合のみ `None; Secure` を付けます。通常のセッション Cookie は `Lax` のままで十分な場合が多いです。
...
成功すると、`document.cookie` / `localStorage` の中身が攻撃者のサーバーに送信されたり、ユーザーになりすまして投稿されたりします。

#### 防御の基本

- **出力をエスケープする**: `textContent` を使う。React / Vue のテンプレート機能に任せる（フレームワークが自動でエスケープしてくれる）
...
- **`SameSite=Lax` / `Strict`**: クロスサイトの POST で Cookie を送らせない。主要ブラウザ（Chrome / Firefox）のデフォルトが `Lax` なので、大半の単純な CSRF は既にブロック済み
- **CSRF トークン**: フォーム送信のたびにサーバーから一意のトークンを渡し、リクエスト時に一緒に送らせる。攻撃者の罠サイトはトークンを知らないので送れない
- **重要操作の再認証**: パスワード変更や送金では、現行セッションでも再度パスワードを入れさせる

現代のフレームワーク（Next.js の Server Actions など）は CSRF トークン処理を内蔵していることが多いため、自分で手書きする機会は減っていますが、仕組みは知っておく価値があります。
...
> **`includeSubDomains` の罠**: 一度配信すると、ブラウザはこのドメインの **全サブドメインも HTTPS 必須** として `max-age` の期間（上の例では 1 年）覚え続けます。社内ツールなど一部サブドメインに HTTPS が無いと、その期間アクセスできなくなります。解除するには **HTTPS で `max-age=0` を配信** します。ただし対象サブドメインに HTTPS が一切無いと、解除ヘッダ自体を届けられず期間が切れるまで戻せません。本番投入は `max-age` を短めから始めて段階的に伸ばすのが安全です。
...
もしあるサイトでセッション系の Cookie が `document.cookie` から見えてしまうなら、そのサイトは XSS でセッションを盗まれるリスクが相対的に高い、という見方ができます（もちろん、他人のサイトで攻撃を試すのは厳禁です。眺めるだけにします）。
...
- DevTools Application タブ → Cookies で、サイトが発行した Cookie の名前・値・属性（`HttpOnly` / `Secure` / `SameSite`）が一覧できる
- セッション系の Cookie には `HttpOnly` と `Secure` のチェックが付いている
- `HttpOnly` 付きの Cookie は、Console で `document.cookie` を実行しても出力に現れない

### 変える

- Cookie の削除・書き換えを試す対象は、**本コースの教材サイト / 自分のローカルプロジェクト / StackBlitz プレビュー** などにとどめます。他人のサイトの Cookie を書き換える行為は不正アクセスと紙一重なので行いません。
...
- Next.js の Server Actions で POST を送る際、DevTools の Network タブで CSRF 関連ヘッダがどう付いているか観察する（発展課題）
...
::: tip この章のドリルで力試し
[6 章「ブラウザの仕組み」のドリル →](/quiz/chapter6/) で、4 択問題で理解度を確認できます。回答履歴はブラウザに保存されるので、途中で閉じても続きから再開できます。
:::
