...
そうした用途（アプリ全体の TODO リストや在庫など、頻繁に変わる大きな state の管理）では Zustand / Redux など専用のライブラリが使われますが、**本コースでは扱いません**。今回は「テーマ切替」という変化の少ない題材に絞ります。
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  // value はオブジェクトリテラルなので毎レンダリングで新参照になる。
  // useMemo で参照を安定化し、配下の useTheme 利用コンポーネントの
  // 不要な再レンダリングを防ぐ。
...
ここで使っている `useMemo` は **「`useMemo` で計算のメモ化」で詳しく扱います**。今は「Context に渡す値の参照を毎回変えないようにする道具」とだけ覚えておけば十分です。
...
- **`value` は `useMemo` で安定化** しています。`value={{ theme, toggleTheme }}` のように直書きすると `ThemeProvider` の再レンダリングごとに新しいオブジェクト参照ができ、配下の `useTheme()` 利用者が **値が変わっていないのに** 全員再レンダリングされてしまうため
