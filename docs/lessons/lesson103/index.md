...
- `global.fetch = vi.fn(...)` の **グローバル差し替え**: `fetch` を関数モックで上書き。手軽だが、レスポンスを毎回手書きすることになり、テストが増えると管理しづらい
...
> **補足: `role="alert"` を後挿入で使うときの注意**: `role="alert"` を持つ要素は **live region** として扱われ、内容が更新されるとスクリーンリーダーが即座に読み上げます。ただし、上のコードのようにエラー発生時にはじめて `<p role="alert">` を **新しく描画** する形は、スクリーンリーダーの実装によっては読み上げが発火しないことがあります。確実に通知したいときは「常設の `<div role="alert">` を空で置いておき、中身だけ差し替える」「`aria-live="assertive"` を併記する」形を検討します。
