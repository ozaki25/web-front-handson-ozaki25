...
画面が変化するアプリには、必ず「状態」があります。TODO の件数、ログインしているかどうか、入力欄の文字、選んでいるタブ、といったものがそれにあたります。

素の JS では、「表示されている DOM の値」が状態の置き場でした。例えばカウンターなら `<p id="count">0</p>` の中のテキストを読んで、書き換えて、と DOM を直接操作していました。「表示」と「状態の管理」が同じ場所に混在している形です。

React では発想が変わります。**状態はメモリ上（JS の変数）で管理し、その状態を元に UI を毎回組み立て直す**という考え方です。DOM の見た目は「状態から計算された結果」にすぎず、開発者が直接触る必要はありません。

```
素の JS: DOM を直接読み書きして状態を管理する
React:   状態（メモリ）→ React が UI を組み立て → DOM に反映
```

この「状態の置き場」を扱う仕組みが **state** で、React ではフックと呼ばれる関数で操作します。本レッスンで使うのが `useState` です。
...
> **`<>...</>` はフラグメント**: 「JSX を書く」で学んだとおり、コンポーネントは 1 つの要素しか return できないため、親タグを増やさずに複数の要素を並べたいときは `<>` と `</>` で囲みます（`<React.Fragment>` の短縮記法）。`<div>` で囲んでも動きますが、不要な `<div>` が増えると CSS や a11y の構造が崩れるため、フラグメントを使うのが基本です。
...
この挙動の理由は、React が **同じイベント内で呼ばれた `setX` をまとめて（バッチで）処理する** ためです。処理するタイミングまで、`count` の値は「このイベントが始まった時点の値」のままになります。
...
上の「3 回呼んでも 1 しか増えない」は、`onClick` のような **同期のイベントハンドラの中** で 3 回連続で `setCount` を呼んだケースです。非同期処理（`setTimeout` や `await` のあと）から `setCount` を呼ぶ場合でも React 18 以降は原則バッチングされますが、条件によってはイベントの境界が切れ、1 回の `setCount(count + 1)` で素直に 1 増える挙動になります。挙動に違和感があったら、まずは関数形式 `setX((prev) => ...)` に書き換えて安全側に倒すのが無難です。
