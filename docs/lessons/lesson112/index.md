# lesson112: ESLint / Prettier / Biome
...
ESLint は「未使用変数があるよ」「`any` 型は避けて」と教え、Prettier は「シングルクォートに統一して、80 文字で改行して」と整えます。両方を併用することで、品質と見た目の両面を担保できます。

歴史的には ESLint だけで両方やる時代もありましたが、**役割を分ける** のが現代の合意になっています。さらに **Biome** という「両方を 1 ツールでやる」統合型の選択肢、ESLint より高速な **oxlint** という Rust 製の lint 単機能型の選択肢も増えており、2026 年は過渡期にあります。
...
リンター・フォーマッタの分野は 2024〜2026 年にかけて大きく動いています。Biome は 2023〜2024 年に「ESLint + Prettier の代替」として注目されました。しかしその後、Rust 系のもうひとつの潮流である oxlint（後述）が急速にシェアを伸ばし、いまの Biome は **「lint + format を 1 ツールで」という統合の利便性を強みに踏ん張る局面** にあります。「これさえ入れればよい」と言える勝者はまだいません。次の 3 通りから自分の事情に合うものを選ぶのが現実的です。

#### A. ESLint + Prettier（依然として最大派閥）

プラグイン資産が圧倒的で、TypeScript の型情報を使う高度なルール（`no-floating-promises` など）も使えます。書きやすさより安全性を優先するならこの組み合わせが向きます。React Native / 大規模モノレポ / 複雑なルール要件があるプロジェクトはほぼこれを選んでいます。

#### B. Biome（lint + format の統合）

設定 1 ファイル / 1 パッケージ / Rust 製で速い構成です。**「lint と format を別ツールで管理する手間を省きたい」** というニーズに応えます。新規の小〜中規模プロジェクトで割り切れる場合は今でも有力です。
...
oxlint は ESLint より **数十倍速い** Rust 製リンターで、ESLint のルール互換性を意識して開発されています。Vue / Vite / Shopify など主要 OSS が次々採用しており、2025 年以降に急伸した本命候補です。フォーマットは Prettier に任せて、lint だけ高速化するハイブリッドが現実解になりつつあります。oxlint 自体は「次世代ツールチェイン」のレッスンで扱います。
...
`git commit` のたびに、ステージングされたファイルだけを対象に Lint + Format が走ります。**全ファイルを毎回チェックしないので速い** のが lint-staged の利点です。
