...
| **`POST`** | 作成・任意の操作 | なし | あり |
| **`PUT`** | 全体置換 | あり | あり |
| **`PATCH`** | 部分更新（特定フィールドだけ書き換え） | 実装依存 | あり |
| **`DELETE`** | 削除 | あり | 基本なし |

他にも `HEAD`（ヘッダだけ取得）/ `OPTIONS`（CORS の事前問い合わせ）がありますが、ブラウザが自動で送るものなのでアプリ側で書く機会は少ないです。

**冪等性**（idempotent） とは「同じリクエストを何回送っても結果が同じ」という性質です。ネットワーク不良で再送されても安全な `GET` / `PUT` / `DELETE` と、再送で二重登録になる恐れがある `POST` は別物として扱われます。`PATCH` は実装によって冪等にも非冪等にもなります（`{ "name": "山田" }` のような置換は冪等、`{ "count_increment": 1 }` のような相対更新は非冪等）。
...
- メソッドは `GET` / `POST` / `PUT` / `PATCH` / `DELETE` の 5 つを基本に、冪等性を意識して使う
