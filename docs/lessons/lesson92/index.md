...
- HTTP のリクエスト / レスポンスがそれぞれ「数行の文字列が連なったもの」であることを理解する
...
1 行目は **リクエストライン** で、`HTTP メソッド パス HTTP バージョン` の 3 つが並びます。2 行目以降は **ヘッダ**（`キー: 値` の形）です。空行が 1 つ入ったあと、必要ならリクエストボディが続きます（GET では普通は付けません）。
...
1 行目は **ステータスライン** で、`HTTP バージョン ステータスコード 理由フレーズ` が並びます。2 行目以降は **ヘッダ**、空行の後に **ボディ**（HTML / JSON / 画像バイナリなど）が続きます。

この 2 つのかたまりがブラウザとサーバーのあいだを **1 往復する** のが HTTP 通信の基本単位です。HTTPS の場合も、TLS で暗号化されるだけで中身の形は同じです。

### 5 つの HTTP メソッド

まずこの 5 つを覚えておけば、現代のアプリの通信はほぼ読めます。
...
**冪等性**（idempotent）とは「同じリクエストを何回送っても結果が同じ」という性質です。ネットワーク不良で再送されても結果が変わらない `GET` / `PUT` / `DELETE` と、再送で二重登録になる恐れがある `POST` は別物として扱われます。`PATCH` も仕様上「冪等である保証はない」とされており（MDN の HTTP メソッド表でも `POST` と並んで Idempotent: No）、`POST` と同じく「再送で結果が変わりうる」側として扱うのが安全です。実装次第で冪等にできなくはないですが、原則は **冪等ではない** と覚えます。
...
| **3xx リダイレクト / キャッシュ** | 別の URL への誘導、またはキャッシュ利用の指示 | `301 Moved Permanently` / `302 Found` / `304 Not Modified` |
...
| `Referer` | どのページから来たか（正しい綴りは referrer だが、綴り間違いのまま仕様になっている） |
...
実物を目で見るのが一番早いので、本レッスンの演習は主にここで手を動かします。
