...
### lesson19 の分割代入と同じ書き方

本題の前に、1 行で接続しておきます。

lesson19 で学んだオブジェクトの分割代入を思い出してください。
...
React コンポーネントの props も、**全く同じ書き方**で値を取り出します。「lesson19 の分割代入の再登場」と思ってください。
...
TypeScript で書くときは、props の形を型で表します。章 3 lesson28 で学んだ `type` エイリアスをそのまま使います。
...
章 3 の lesson28 / lesson31 で `types.ts` に `Todo` や `GreetingProps` のような型を書き、`import type` で呼ぶ練習をしました。React でも同じやり方が使えます。
...
`{age !== undefined && <p>{age} 歳です</p>}` の `&&` は「左が真なら右を表示」。条件表示の詳しい話は lesson39 で扱います。ここでは「`age` が省略されたら `<p>` は出ない」と読み取れれば OK です。
...
1. StackBlitz の React + Vite（TS）テンプレートから新規プロジェクトを作る（lesson33 のを使い回しても OK）
