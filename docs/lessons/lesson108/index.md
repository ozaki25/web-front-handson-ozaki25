...
並列実行は `&` ではなく `npm-run-all` / `concurrently` を使うのが安全です。シェルの `&` でバックグラウンド化すると次の問題があります。

- **Windows で動かない**: `&` は Unix のバックグラウンド実行だが、Windows の cmd では「コマンド区切り」の意味になり挙動が違う
- **片方が落ちても気づかない**: 子プロセスの終了コードを拾えず、ビルドが部分的に失敗しても成功扱いになる
- **Ctrl+C で全部止まらない**: 親プロセスを切っても子が残り続けることがある
- **ログが混ざる**: どちらの出力か区別できない

`concurrently` を使うと、片方が落ちたら全部止める（`--kill-others-on-fail`）、ログにプレフィックスを付ける、Ctrl+C で全プロセスを正しく終了する、といった面倒を見てくれます。
...
`npm:dev:*` は「`dev:` で始まる scripts をすべて並列実行」というショートハンドです。
