# lesson79: エラーと見つからないページ

## ゴール

- レンダリング中に発生した例外を `error.tsx` で捕まえて、壊れた画面の代わりにエラー画面を出せる
- `notFound()` を呼んで `not-found.tsx` を表示できる
- `error.tsx` が担当する範囲と、Server Actions のフォームエラー（「送信状態とエラー表示」）の担当範囲が **別物** であることを理解する
...
- フォーム入力エラーはこの 2 つでは扱わない。「送信状態とエラー表示」の `useActionState` を使う
