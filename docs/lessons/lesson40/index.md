...
items.sort(); // コードポイント順（英大文字が先、カタカナがひらがなの後ろに来る）
// => ["Apple", "みかん", "りんご", "バナナ"]

items.sort(new Intl.Collator("ja").compare);
// => ["Apple", "バナナ", "みかん", "りんご"]（日本語の五十音の並び）
```
...
// => ["Äpfel", "apple", "banana"]（ä を a と同列に扱って並べる。素の sort() では Ä が最後に飛ばされる）
```
...
この演習は Vite の TypeScript テンプレートを使います。コードに出てくる `as ...` や `!` は TypeScript の記法ですが、3 章で学ぶので今はコピペで動かして構いません。
