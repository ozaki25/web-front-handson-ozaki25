...
- 最低限のアクセシビリティ（見出し階層・コントラスト・フォーカスリング）を意識した実装ができる

別のレッスンで、この `.cards` 部分を **CSS Grid** に書き換えます。2 次元レイアウトが本来の Grid の得意分野で、`@media` を書かなくても画面幅に応じて自動で折り返す形にできます。

ここで作った **自己紹介ページ** は、5 章 の「ページを増やしてリンクで移動する」で Next.js の `/about` ページとして再利用します。HTML / CSS のファイルはそのまま保存しておきましょう。**`class` を `className` に、`<label for>` を `<label htmlFor>` に、自己閉じタグに `/` を足すだけ** で、ほぼそのまま JSX になります。
