...
`margin` も書き方は同じです。値にマイナスも指定できますが、使い所は限られるので本コースでは正の値のみ扱います。
...
3. 試しに、ファイル先頭の `box-sizing: border-box;` のルールを **一時的に削除** して保存する。`.card` の `padding: 24px` が加算されて、800px の制限を超える幅になる（横スクロールが出る場合あり）。`border-box` の効果がこの差分で確認できる。確認後は必ず戻す。
