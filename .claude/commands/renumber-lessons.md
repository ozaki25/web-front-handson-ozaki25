...
引数: `$ARGUMENTS`（例: `12:15 13:16 14:17 ...` のような `旧:新` ペアの空白区切り。または `docs/drafts/renumber-map.txt` のパス）

## 事前確認

1. ユーザーから受け取った旧→新のマッピングを確認する
2. **衝突チェック**: 新番号の中に、まだ変換されていない旧番号と重複するものがないか検証（降順処理で安全か確認）
3. 現在のブランチが `draft` か `publish/lessonXX` であること（main では docs/lessons を直接編集できない）
...
- **main ブランチでは実行しない**: CLAUDE.md のブランチ運用に従い、draft または publish/lessonXX 上で実行
