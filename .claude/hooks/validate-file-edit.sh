...
# 許可外ブランチでの編集をすべてブロック
if [[ "$BRANCH" != "main" && "$BRANCH" != "draft" && ! "$BRANCH" =~ ^publish/lesson[0-9]{2}$ ]]; then
  block "ブランチ運用違反: 現在のブランチ '$BRANCH' は許可されていません。main または draft に切り替えてください"
fi

# main でコンテンツファイルを編集しようとしている
if [ "$BRANCH" = "main" ]; then
  if [[ "$FILE_PATH" =~ docs/lessons/lesson[0-9]+/ ]] || [[ "$FILE_PATH" =~ docs/drafts/ ]] || [[ "$FILE_PATH" =~ docs/rejected/ ]]; then
...
if [[ "$BRANCH" =~ ^publish/lesson[0-9]{2}$ ]]; then
  if [[ "$FILE_PATH" =~ docs/lessons/lesson[0-9]+/ ]] || [[ "$FILE_PATH" =~ docs/\.vitepress/config\.mts ]] || [[ "$FILE_PATH" =~ docs/index\.md ]]; then
    : # 許可
  else
    block "ブランチ運用違反: publish ブランチでは該当レッスン、サイドバー、index.md のみ変更できます。対象: $FILE_PATH"
  fi
fi
